...
if __name__ == '__main__':
    model = YOLOv10()  # 指定YOLO模型对象，并加载指定配置文件中的模型配置
    model.load('yolov10s.pt')      #加载预训练的权重文件'yolov10s.pt'，加速训练并提升模型性能
    model.train(data='D:/Software/Pycharm/yolov10-main/ultralytics/cfg/datasets/NEU-DET.yaml',  # 指定训练数据集的配置文件路径，这个.yaml文件包含了数据集的路径和类别信息
